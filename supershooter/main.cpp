...
protected:
    virtual bool mouse_down(int button, float x,float y) {
        ...
        else if (state==IN_MENU and main_menu->is_selected("music")) { SoundManager::get()->toggle_musics(); }
        else if (state==IN_MENU and main_menu->is_selected("sfx")) { SoundManager::get()->toggle_sfxs(); }
    ...
    virtual bool key_down(SDLKey key) {
        if (key==SDLK_m) { SoundManager::get()->toggle_musics(); return true; }
        if (key==SDLK_s) { SoundManager::get()->toggle_sfxs(); return true; }
