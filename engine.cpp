...
SdlManager::SdlManager(int w,int h,int d) : in_main_loop(false), width(w), height(h), old_ticks(0) {
    if (SDL_Init(SDL_INIT_VIDEO|SDL_INIT_AUDIO|SDL_INIT_TIMER|SDL_OPENGL)) { std::cerr<<"cannot initialize sdl..."<<std::endl; throw Except(Except::SS_INIT_ERR); }

    screen=SDL_SetVideoMode(width,height,d,SDL_OPENGL|SDL_DOUBLEBUF);
    if (not screen) { std::cerr<<"cannot create sdl screen..."<<std::endl; throw Except(Except::SS_INIT_ERR); }
